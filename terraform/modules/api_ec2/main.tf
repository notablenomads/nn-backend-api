...
resource "aws_launch_template" "api" {
  ...
  user_data = base64encode(<<-EOF
              ...
              # Verify GEMINI_API_KEY specifically
              echo "Checking GEMINI_API_KEY in SSM response:"
              cat /etc/api/env.json | jq '.[] | select(.Name | endswith("GEMINI_API_KEY")) | .Name'
              
              # Verify GEMINI_API_KEY in environment file
              echo "Checking GEMINI_API_KEY in environment file:"
              if grep -q "GEMINI_API_KEY" /etc/api/container.env; then
                echo "GEMINI_API_KEY is present in environment file"
              else
                echo "ERROR: GEMINI_API_KEY is missing from environment file"
